...
public class CourseService {
  ...
  @Cacheable(
      value = "courses",
      key = "#pageable.pageNumber + '_' + #pageable.pageSize + '_' + #filters.hashCode()")
  ...
  public CourseResponseDTO getCourseById(String courseId, String userId) {
    ...
    dto.setTotalLessons(
        course.getModules().stream().mapToInt(module -> module.getLessons().size()).sum());
  ...
  public CourseResponseDTO createCourse(CourseRequestDTO courseRequest, String userId) {
    ...
    User instructor =
        userRepository
            .findById(userId)
            .orElseThrow(
                () -> new ResourceNotFoundException("Instructor not found with ID: " + userId));
    // Check for duplicate course title for this instructor
    boolean titleExists =
        courseRepository.existsByTitleAndInstructorId(courseRequest.getTitle(), userId);
    if (titleExists) {
  ...
  public CourseStatisticsDTO getCourseStatistics(String courseId) {
    ...
    List<Enrollment> enrollments =
        enrollmentRepository.findByCourseId(courseId, Pageable.unpaged()).toList();
    ...
    double completionRate =
        enrollments.isEmpty() ? 0.0 : (completedEnrollments * 100.0) / enrollments.size();

    // Calculate revenue
    BigDecimal totalRevenue = course.getPrice().multiply(BigDecimal.valueOf(enrollments.size()));
    BigDecimal platformCommission =
        totalRevenue.multiply(BigDecimal.valueOf(0.30)); // 30% platform fee
    BigDecimal instructorEarnings = totalRevenue.subtract(platformCommission);

    // Calculate engagement metrics
    double averageProgress =
        enrollments.stream().map(Enrollment::getProgressPercentage).mapToDouble(BigDecimal::doubleValue).average().orElse(0.0);

    // Get rating distribution
    Map<Integer, Integer> ratingDistribution =
        reviewService.getRatingDistributionForCourse(courseId);

    // Get recent enrollments
    LocalDateTime oneWeekAgo = LocalDateTime.now().minusDays(7);
    LocalDateTime oneMonthAgo = LocalDateTime.now().minusDays(30);

    long enrollmentsLast7Days =
        enrollments.stream().filter(e -> e.getEnrollmentDate().isAfter(oneWeekAgo)).count();
    long enrollmentsLast30Days =
        enrollments.stream().filter(e -> e.getEnrollmentDate().isAfter(oneMonthAgo)).count();
    ...
    return CourseStatisticsDTO.builder()
        ...
        .averageRating(course.getAverageRating().doubleValue())
  ...
  public List<CourseListDTO> getPopularCourses(int limit) {
    ...
    return courses.stream().map(courseMapper::toListDTO).collect(Collectors.toList());
  }
  ...
  private void enrichWithUserSpecificData(CourseResponseDTO dto, String userId, String courseId) {
    ...
    if (isEnrolled) {
      enrollmentRepository
          ...
          .ifPresent(
              enrollment -> {
                dto.setUserProgress(enrollment.getProgressPercentage().doubleValue());
              });
    }
  ...
  private String generateRatingStars(BigDecimal rating) {
    if (rating == null || rating.doubleValue() == 0) {
      return "☆☆☆☆☆";
    }

    int fullStars = (int) Math.floor(rating.doubleValue());
    boolean hasHalfStar = (rating.doubleValue() - fullStars) >= 0.5;
  ...
  private boolean isAuthorizedToViewUnpublished(String userId, Course course) {
    ...
    return userRepository
        .findById(userId)
        .map(user -> user.getRoles().stream().anyMatch(role -> role.getName().equals("ROLE_ADMIN")))
        .orElse(false);
  }
    ...
    activeEnrollments.forEach(
        enrollment -> {
          emailService.sendCourseDeletionNotification(
              enrollment.getUser().getEmail(), course.getTitle());
        });
  }
    ...
    wishlistItems.forEach(
        wishlist -> {
          emailService.sendCoursePublishedNotification(
              wishlist.getUser().getEmail(), course.getTitle());
        });
  }
