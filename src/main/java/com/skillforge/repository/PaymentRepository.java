...
public interface PaymentRepository extends JpaRepository<Payment, String> {
    ...
    @Query("SELECT p FROM Payment p WHERE p.user.id = :userId AND p.course.id = :courseId AND p.status = 'COMPLETED'")
    Optional<Payment> findSuccessfulPayment(@Param("userId") String userId, @Param("courseId") String courseId);
    ...
    @Modifying
    @Query("UPDATE Payment p SET p.status = 'COMPLETED', p.paymentDate = CURRENT_TIMESTAMP " +
            "WHERE p.id = :paymentId AND p.status = 'PENDING'")
    int markAsCompleted(@Param("paymentId") String paymentId);

    @Modifying
    @Query("UPDATE Payment p SET p.status = 'REFUNDED', p.refundDate = CURRENT_TIMESTAMP, p.refundReason = :reason " +
            "WHERE p.id = :paymentId")
    void refundPayment(@Param("paymentId") String paymentId, @Param("reason") String reason);

    @Query("SELECT FUNCTION('MONTH', p.paymentDate), COALESCE(SUM(p.amount), 0) FROM Payment p " +
