...
public interface EnrollmentRepository extends JpaRepository<Enrollment, String> {
  ...
  // Progress tracking
  @Query(
      "SELECT AVG(e.progressPercentage) FROM Enrollment e WHERE e.course.id = :courseId AND e.status = 'ACTIVE'")
  Double getAverageProgressForCourse(@Param("courseId") String courseId);

  @Query(
      "SELECT COUNT(e) FROM Enrollment e WHERE e.course.id = :courseId AND e.progressPercentage = 100")
  long countCompletionsByCourse(@Param("courseId") String courseId);

  // Completion rate
  @Query(
      "SELECT (COUNT(CASE WHEN e.progressPercentage = 100 THEN 1 END) * 100.0 / COUNT(e)) "
          + "FROM Enrollment e WHERE e.course.id = :courseId")
  Double getCompletionRateForCourse(@Param("courseId") String courseId);

  // Recent enrollments
  @Query(
      "SELECT e FROM Enrollment e WHERE e.enrollmentDate > :since ORDER BY e.enrollmentDate DESC")
  ...
  @Query(
      "SELECT e.course.id, COUNT(e) FROM Enrollment e WHERE e.status = 'DROPPED' GROUP BY e.course.id")
  ...
  @Query(
      "SELECT COUNT(e) FROM Enrollment e WHERE e.course.instructor.id = :instructorId "
          + "AND e.enrollmentDate BETWEEN :startDate AND :endDate")
  long countByCourseInstructorIdAndEnrollmentDateBetween(
  ...
  long countByCourseInstructorIdAndEnrollmentDateAfter(
      @Param("instructorId") String instructorId, @Param("since") LocalDateTime since);
