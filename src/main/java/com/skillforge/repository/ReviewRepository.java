...
public interface ReviewRepository extends JpaRepository<Review, String> {
  ...
  @Modifying
  @Query(
      "UPDATE Review r SET r.instructorReply = :reply, r.replyDate = CURRENT_TIMESTAMP "
          + "WHERE r.id = :reviewId")
  void addInstructorReply(@Param("reviewId") String reviewId, @Param("reply") String reply);

  @Query(
      "SELECT r FROM Review r WHERE r.isVerifiedPurchase = false AND r.course.instructor.id = :instructorId")
