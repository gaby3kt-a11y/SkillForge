...
public interface ModuleRepository extends JpaRepository<Module, String> {
  ...
  @Modifying
  @Query(
      "UPDATE Module m SET m.orderIndex = m.orderIndex + 1 WHERE m.course.id = :courseId AND m.orderIndex >= :fromIndex")
  ...
  @Query(
      "SELECT COALESCE(SUM(l.durationMinutes), 0) FROM Module m JOIN m.lessons l WHERE m.course.id = :courseId")
