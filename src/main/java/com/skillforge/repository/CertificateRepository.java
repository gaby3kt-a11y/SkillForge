...
public interface CertificateRepository extends JpaRepository<Certificate, String> {
  ...
  Optional<Certificate> findByUserAndCourse(
  ...
  @Query(
      "SELECT c FROM Certificate c WHERE c.issueDate < CURRENT_TIMESTAMP AND c.expiryDate < CURRENT_TIMESTAMP")
  List<Certificate> findExpiredCertificates();

  @Query(
      "SELECT c.course.id, COUNT(c) FROM Certificate c GROUP BY c.course.id ORDER BY COUNT(c) DESC")
